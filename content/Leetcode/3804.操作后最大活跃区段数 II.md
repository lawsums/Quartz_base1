...
> 按学习顺序排列，覆盖线段树在力扣上的主流考法。

| # | 题目 | 难度 | 线段树用法 | 链接 |
|---|------|------|-----------|------|
| 1 | 307. Range Sum Query - Mutable | Medium | **入门**：单点更新 + 区间求和，最基础的线段树（也适合树状数组） | https://leetcode.com/problems/range-sum-query-mutable/ |
| 2 | 2407. Longest Increasing Subsequence II | Hard | **区间最大值 + DP**：线段树维护 DP 值的区间 max，代替 $O(k)$ 暴力转移 | https://leetcode.com/problems/longest-increasing-subsequence-ii/ |
| 3 | 3165. Maximum Sum of Subsequence With Non-adjacent Elements | Hard | **节点存多维信息**：每个节点维护 4 个值 `f00, f01, f10, f11`（左右两端选/不选），合并时需要技巧 | https://leetcode.com/problems/maximum-sum-of-subsequence-with-non-adjacent-elements/ |
| 4 | 2940. Find Building Where Alice and Bob Can Meet | Hard | **线段树上二分**：在区间 max 线段树上做"第一个大于某值的位置"的定向查找 | https://leetcode.com/problems/find-building-where-alice-and-bob-can-meet/ |

**练习路径建议：**

1. **307** — 先把 `build / update / query` 模板写熟。
2. **2407** — 学会用线段树加速 DP（代替 $O(n)$ 的线性扫描）。
3. **3165** — 学会在 `merge` 中维护复杂信息（不像 3804 只是 `max`，这题每个节点要维护 4 个状态，合并逻辑需要推导）。
4. **2940** — 学会在线段树上"行走"（二分搜索），不同于标准的区间查询。

这四道 + 本题 3804 一起，基本覆盖了力扣上线段树的全部考法。
